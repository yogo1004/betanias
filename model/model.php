...
function getMeetingByDate($date_meeting){
    $query = "SELECT * from meetings where date_meeting = :date_meeting";
    $execute = [
...
function getResponsiblesbyBetania($id_betania){
    $query = "SELECT firstname from betanias
    JOIN responsibles ON betanias.id_betania = responsibles.Betanias_id_betania
     WHERE id_betania =:id_betania";

    $execute = [
        'id_betania' => $id_betania
    ];
   return getFetchAllById($query, $execute);

}function getMeetingsbyBetania($id_betania){
    $query = "SELECT date_meeting, friends, Siblings, childrens, offering from betanias
    JOIN meetings ON betanias.id_betania = meetings.Betanias_id_betania
    WHERE id_betania = :id_betania ORDER BY date_meeting DESC";
...
function getFetchAllById($query,$execute)
{
    require "model/.constant.php";
    try {
        $dbh = getPDO();
        $statment = $dbh->prepare($query);//prepare query, il doit faire des vérifications et il va pas exécuter tant
        //qu'il y a des choses incorrects
        $statment->execute($execute);//execute query
        $queryResult = $statment->fetchAll(PDO::FETCH_ASSOC);//prepare result for client cherche tous les résultats
        $dbh = null; //refermer une connection quand on a fini
        if ($debug) var_dump($queryResult);
        return $queryResult;
...
function getFetchById($query,$execute)
{
    require "model/.constant.php";
    try {
        $dbh = getPDO();
